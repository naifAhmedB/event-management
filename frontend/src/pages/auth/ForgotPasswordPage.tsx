...
const ForgotPasswordPage = () => {
  ...
  const onSubmit = async (data: ForgotForm) => {
    setLoading(true);
    try {
      const res = await authService.requestOtp(data.phone, selectedMethod, 'forgot_password');
      if (res.success) {
        sessionStorage.setItem('fp_phone', data.phone);
        sessionStorage.setItem('fp_method', selectedMethod);
        toast.success(selectedMethod === 'whatsapp' ? t('auth.otp_sent_whatsapp') : t('auth.otp_sent_sms'));
        navigate('/signup/verify?purpose=forgot_password');
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-purple-50 to-indigo-50 p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-2">{t('auth.forgot_password')}</h2>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Input
              ...
              {...register('phone', {
                ...
                pattern: { value: /^(05|5|\+9665)\d{8}$/, message: t('common.invalid_phone') },
              })}
            />
